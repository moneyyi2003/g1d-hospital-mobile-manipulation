...
The survey poses are never model inputs. They are consumed only after RGB-only
inference to establish the metric map frame used by occupancy and SAM3 projection.
"""
...
def fit_similarity(source, target):
    ...
    if source.shape != target.shape or source.ndim != 2 or source.shape[1] != 3:
        raise ValueError("Similarity correspondences must both have shape [N, 3]")
    if source.shape[0] < 3 or not np.isfinite(source).all() or not np.isfinite(target).all():
        raise ValueError("Similarity fitting needs at least three finite correspondences")
    ...
    if variance <= 1e-12:
        raise ValueError("LingBot camera trajectory is degenerate")
    ...
    if not math.isfinite(scale) or scale <= 0.0:
        raise ValueError("Similarity fitting produced an invalid scale")
...
def _predicted_camera_centres(prediction_dir: Path):
    ...
    if not files:
        raise ValueError(f"No LingBot predictions found in {prediction_dir}")
    centres = []
    for path in files:
        with np.load(path, allow_pickle=False) as data:
            key = "camera_to_world" if "camera_to_world" in data else "extrinsic"
            if key not in data:
                raise ValueError(f"{path.name} has no predicted camera transform")
...
def _survey_camera_centres(manifest_path: Path):
    ...
    try:
        ...
        raise ValueError(f"Cannot read RGB survey manifest {manifest_path}: {exc}") from exc
    if centres.ndim != 2 or centres.shape[1] != 3 or not np.isfinite(centres).all():
        raise ValueError("Survey camera positions must have shape [N, 3]")
...
def align_lingbot_to_survey(
    ...
    *,
    ...
    if not math.isfinite(inlier_threshold_m) or inlier_threshold_m <= 0.0:
        raise ValueError("Alignment inlier threshold must be positive")
...
def build_pose_anchored_alignment(
    ...
    *,
    ...
    if not 0.0 <= confidence_quantile < 1.0:
        raise ValueError("Floor confidence quantile must be in [0, 1)")
    ...
    if int(inliers.sum()) < max(3, len(files) // 2):
        raise ValueError("Floor-derived LingBot scales are not stable enough")
