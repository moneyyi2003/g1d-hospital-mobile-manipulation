...
def load_ros_occupancy(map_yaml: str | Path) -> OccupancyGrid:
    ...
    # Parse the YAML manually for the PGM and metadata
    yaml_path = Path(map_yaml)
    yaml_dir = yaml_path.parent
    with yaml_path.open("r", encoding="utf-8") as f:
        yaml_text = f.read()

    # Simple YAML parser for the map metadata format
    metadata = {}
    ...
    pgm_path = yaml_dir / image_rel
    if not pgm_path.is_file():
        pgm_path = Path(image_rel)
    with pgm_path.open("rb") as f:
        header = f.readline().decode("ascii").strip()
        if header != "P5":
            raise ValueError(f"Expected P5 PGM, got {header}")
    ...
    origin = [float(x) for x in metadata.get("origin", "0 0 0").replace("[", "").replace("]", "").split()]
    ...
    occupancy = pixels / 255.0 if negate else (255 - pixels) / 255.0
    cells = np.full((height, width), -1, dtype=np.int8)
    cells[occupancy < free_thresh] = 0
    cells[occupancy > occupied_thresh] = 100
    cells = np.flipud(cells)

    from .occupancy import OccupancyBuildConfig
    return OccupancyGrid(
        cells, resolution, origin[0], origin[1],
        OccupancyBuildConfig(resolution=resolution),
    )
...
class SemanticMapConfig:
    ...
    def validate(self) -> None:
        if not self.labels or any(not str(label).strip() for label in self.labels):
            raise ValueError("Semantic labels must not be empty")
        if self.frame_stride < 1 or self.pixel_stride < 1:
            raise ValueError("Semantic frame/pixel strides must be positive")
...
class ClipSegSegmenter:
    ...
    def __init__(self, model_id: str = "CIDAS/clipseg-rd64-refined", device: str = "cuda"):
        try:
            ...
            raise ImportError("CLIPSeg needs torch and transformers") from exc
        self.torch = torch
...
def _regions_from_free(cells):
    ...
    try:
        ...
        raise ImportError("Region extraction needs scipy") from exc
...
def build_semantic_maps(
    ...
    *,
    ...
    alignment_matrix,
    segmenter: Callable,
    config: SemanticMapConfig | None = None,
) -> dict:
    if config is None:
    ...
    if alignment.shape != (4, 4) or not np.isfinite(alignment).all():
        raise ValueError("alignment_matrix must be a finite 4x4 matrix")
    root, output = Path(prediction_dir), Path(output_dir)
    files = sorted(root.glob("frame_*.npz"))[:: config.frame_stride]
    if not files:
        raise ValueError(f"No LingBot prediction frames under {root}")
    ...
    for path in files:
        with np.load(path, allow_pickle=False) as data:
            ...
            if not all(key in data for key in required):
                raise ValueError(f"{path.name} lacks compact LingBot RGB-D fields")
            ...
            if scores.shape != (len(config.labels), *depth_shape):
                raise ValueError(f"Segmenter returned unexpected shape {scores.shape}")
