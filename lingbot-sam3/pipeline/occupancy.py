...
class OccupancyBuildConfig:
    ...
    def validate(self) -> None:
        ...
        if not all(math.isfinite(value) for value in values):
            raise ValueError("Occupancy settings contain non-finite values")
        if self.resolution <= 0 or self.ground_band <= 0 or self.padding < 0:
            raise ValueError("Invalid occupancy resolution/ground band/padding")
        if self.obstacle_max_height <= self.obstacle_min_height:
            raise ValueError("obstacle_max_height must exceed obstacle_min_height")
        ...
        if not 0 <= self.bounds_quantile < 0.1:
            raise ValueError("bounds_quantile must be in [0, 0.1)")
...
def build_occupancy(points, config: OccupancyBuildConfig) -> OccupancyGrid:
    ...
    if points.shape[0] < 100:
        raise ValueError("At least 100 finite points are required to build a map")
...
def clear_traversed_footprints(
    ...
    if positions.ndim != 2 or positions.shape[1] != 2 or not np.isfinite(positions).all():
        raise ValueError("Traversed footprint positions must have shape [N, 2]")
    if not math.isfinite(radius_m) or radius_m <= 0.0:
        raise ValueError("Traversed footprint radius must be positive")
