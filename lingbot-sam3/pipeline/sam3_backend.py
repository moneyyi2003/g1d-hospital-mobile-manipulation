"""SAM3 video tracking adapter: video + text prompts → per-frame segmentation masks.

Runs the official SAM3/SAM3.1 model to track text-described objects across
video frames and saves per-frame masks, scores, and boxes as NPZ artifacts.
"""
...
class Sam3TrackConfig:
    ...
    def validate(self) -> None:
        ...
        if not 0.0 <= self.probability_threshold <= 1.0:
            raise ValueError("SAM3 probability threshold must be in [0, 1]")
        if self.propagation_direction not in {"forward", "backward", "both"}:
            raise ValueError("Unsupported SAM3 propagation direction")
        if min(self.grounding_batch_size, self.postprocess_batch_size) < 1:
            raise ValueError("SAM3 batch sizes must be positive")
...
def _save_frame(path: Path, prompt_index: int, outputs: dict[str, Any]) -> int:
    try:
        ...
        raise ImportError("SAM3 artifact serialization requires NumPy") from exc
    ...
    if masks.ndim != 3 or masks.shape[0] != len(object_ids):
        raise ValueError(f"Unexpected SAM3 mask output shape: {masks.shape}")
...
def run_sam3_tracking(
    ...
    *,
    ...
    Args:
        video_resource: Path to MP4 video file or directory of RGB frames.
        prompts: List of text descriptions to track (e.g., ["chair", "table"]).
        output_directory: Where to save per-prompt per-frame NPZ artifacts.
        checkpoint: Path to SAM3.1 checkpoint directory or .pt file.
        config: Tracking configuration.

    Returns:
        Manifest dict with prompt records and pipeline metadata.
    """
    config = config or Sam3TrackConfig()
    config.validate()
    prompt_list = tuple(item.strip() for item in prompts if item.strip())
    if not prompt_list:
        raise ValueError("At least one SAM3 text prompt is required")
    resource = Path(video_resource).expanduser().resolve()
    if not resource.exists():
        raise ValueError(f"SAM3 video resource does not exist: {resource}")
    ...
    try:
        ...
        raise ImportError(
            "SAM3 must be importable. Make sure the sam3 package is in your PYTHONPATH. "
    ...
    if checkpoint is not None:
        ...
        if not checkpoint_path.is_file():
            raise ValueError(f"SAM3 checkpoint does not exist: {checkpoint_path}")
