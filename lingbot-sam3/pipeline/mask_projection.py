"""Project SAM3 masks through LingBot-Map geometry into the map frame."""
...
def project_mask_to_map(
    ...
    *,
    ...
    if not np.isfinite(scale_m_per_unit) or scale_m_per_unit <= 0:
        raise ValueError("LingBot-to-map scale must be explicitly positive")
    ...
    if confidence is not None:
        ...
        if confidence_array.shape != mask_array.shape:
            raise ValueError("LingBot point confidence is not pixel-aligned")
    ...
    if matrix.shape != (4, 4) or not np.isfinite(matrix).all():
        raise ValueError("LingBot-to-map alignment must be a finite 4x4 matrix")
...
def project_pose_anchored_mask_to_map(
    ...
    *,
    ...
    if confidence is not None:
        confidence_array = np.asarray(confidence).squeeze()
        if confidence_array.shape != mask_array.shape:
            raise ValueError("LingBot depth confidence is not pixel-aligned")
        finite = np.isfinite(confidence_array)
        valid &= finite
        if valid.any():
            threshold = np.quantile(confidence_array[valid], confidence_quantile)
    ...
    try:
        ...
        raise ValueError(f"Survey camera pose is invalid: {exc}") from exc
    if position.shape != (3,) or not np.isfinite(position).all():
        raise ValueError("Survey camera position must be a finite xyz vector")
...
def build_track_observations(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"Cannot read alignment {alignment_path}: {exc}"
        ) from exc
    pose_anchored = (
    ...
    for mask_path in sorted(sam_root.glob("frame_*.npz")):
        try:
            ...
            raise ValueError(f"Invalid SAM3 frame artifact name: {mask_path.name}") from exc
        prediction_path = prediction_root / f"frame_{frame_index:06d}.npz"
        if not prediction_path.is_file():
            raise ValueError(f"Missing LingBot geometry for SAM3 frame {frame_index}")
        with np.load(mask_path, allow_pickle=False) as masks, np.load(
            ...
            if pose_anchored:
                ...
                if survey_frames is None or frame_index >= len(survey_frames):
                    raise ValueError(f"Survey manifest lacks frame {frame_index}")
            elif "world_points" not in geometry:
                raise ValueError(f"{prediction_path.name} lacks world_points")
