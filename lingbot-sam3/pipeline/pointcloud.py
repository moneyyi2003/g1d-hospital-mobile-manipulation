"""Convert LingBot-Map per-frame predictions into an aligned RGB point cloud."""
...
class PointCloudBuildConfig:
    ...
    def validate(self) -> None:
        ...
        if matrix.shape != (4, 4) or not np.isfinite(matrix).all():
            raise ValueError("Alignment matrix must be a finite 4x4 matrix")
        if not np.allclose(matrix[3], [0, 0, 0, 1], atol=1e-7):
            raise ValueError("Alignment matrix last row must be [0, 0, 0, 1]")
        if not math.isfinite(self.scale_m_per_unit) or self.scale_m_per_unit <= 0:
            raise ValueError("scale_m_per_unit must be explicitly set and positive")
        ...
        if min(self.frame_stride, self.max_points_per_frame, self.max_total_points) < 1:
            raise ValueError("Point-cloud sampling options must be positive")
...
def load_alignment_matrix(path: str | Path):
    ...
    try:
        ...
        raise ValueError(f"Cannot read alignment matrix {source}: {exc}") from exc
...
def _normalize_colors(images, target_shape):
    ...
    if colors.shape != (*target_shape, 3):
        raise ValueError(
            f"Images shape {colors.shape} does not match points {target_shape}"
        )
...
def _unproject_depth(depth, intrinsic, extrinsic=None, camera_to_world=None):
    """Reconstruct world points from depth map and camera parameters."""
    np = _numpy()
    ...
    if depth.ndim != 2 or intrinsic.shape != (3, 3) or c2w.shape != (4, 4):
        raise ValueError("Depth/intrinsic/camera transform shapes are invalid")
...
def _camera_points_from_depth(depth, intrinsic):
    ...
    if depth.ndim != 2 or intrinsic.shape != (3, 3):
        raise ValueError("Depth/intrinsic shapes are invalid")
    height, width = depth.shape
    rows, cols = np.indices((height, width), dtype=np.float64)
    x = (cols - intrinsic[0, 2]) * depth / intrinsic[0, 0]
    y = (rows - intrinsic[1, 2]) * depth / intrinsic[1, 1]
...
def _quaternion_wxyz_to_rotation(quaternion):
    ...
    if value.shape != (4,) or not np.isfinite(value).all():
        raise ValueError("Survey camera quaternion must be finite wxyz")
    norm = float(np.linalg.norm(value))
    if norm <= 1e-12:
        raise ValueError("Survey camera quaternion has zero length")
    w, x, y, z = value / norm
    return np.asarray(
        [
...
# Survey manifest stores camera orientation for +X forward, +Y left, +Z up.
...
def load_pose_anchored_lingbot_points(
    ...
    if not frame_files:
        raise ValueError(f"No frame_*.npz files found in {root}")
    try:
        manifest = json.loads(Path(survey_manifest).read_text(encoding="utf-8"))
        frames = manifest["frames"]
    except (OSError, json.JSONDecodeError, KeyError, TypeError) as exc:
        raise ValueError(f"Cannot read survey manifest {survey_manifest}: {exc}") from exc
    ...
    for frame_index, (path, frame) in enumerate(zip(frame_files, frames)):
        ...
        with np.load(path, allow_pickle=False) as data:
            ...
            if confidence.shape != target_shape:
                raise ValueError(f"Confidence shape mismatch in {path}: {confidence.shape}")
            ...
            if position.shape != (3,) or not np.isfinite(position).all():
                raise ValueError(f"Survey frame {frame_index} position is invalid")
            camera_to_survey = survey_rotation @ cv_to_survey
            ...
            anchored = selected @ camera_to_survey.T + position
            point_chunks.append(anchored.astype(np.float32))
            color_chunks.append(colors[indices])
            used += 1

    if not point_chunks:
        raise ValueError("No valid LingBot points survived confidence filtering")
    ...
    for frame_index, path in enumerate(frame_files):
        ...
        with np.load(path, allow_pickle=False) as data:
            if "images" not in data:
                raise ValueError(
                    f"{path.name} needs images; rerun LingBot inference first"
                )
            ...
            if points_map.ndim != 3 or points_map.shape[-1] != 3:
                raise ValueError(f"Unexpected world_points shape in {path}: {points_map.shape}")
...
def write_binary_ply(path: str | Path, points, colors) -> None:
    ...
    if points.ndim != 2 or points.shape[1] != 3 or colors.shape != points.shape:
        raise ValueError("PLY points and colors must both have shape [N, 3]")
    vertex = np.empty(
        points.shape[0],
        dtype=[
            ("x", "<f4"),
