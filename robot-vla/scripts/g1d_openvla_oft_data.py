...
def build_manifest(demo_dir: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    for episode in sorted(demo_dir.rglob("episode_*")):
        if not episode.is_dir():
            continue
        relative_parts = episode.relative_to(demo_dir).parts[:-1]
        if any(
            part == "rejected" or part.startswith("expert_run_")
        ...
        try:
            ...
            if not evidence.get("physical_hold_verified"):
                raise ValueError("physical finger hold was not verified")
            if evidence.get("fixed_joint_created") or evidence.get(
                "fixed_joint_configured"
            ):
                raise ValueError("artificial fixed-joint grasp is forbidden")
            if int(evidence.get("hold_contact_frames", 0)) < 30:
                raise ValueError("two-finger contact is shorter than 30 frames")
            ...
            accepted.append(
                {
                    ...
                    "hold_contact_frames": int(evidence["hold_contact_frames"]),
