...
occupancy，并接入 SAM3.1、region、地点审核与网页。2026-07-30 又增加了餐桌、媒体柜
...
正在由新增物品后的 292 帧巡检替换。SAM3 每个自主标签独立落盘并支持断点续跑；只有
全部标签完成投影和人工审核后，新 occupancy/semantic/region/object catalog 才能成为
正式图。正式、针对 G1-D 训练的 VLA 尚未交付；2026-07-30
...
./mobilemanibench.sh home-task \
  --headless --test --resolution 640x360 \
  --command '请带我去客厅沙发旁，拿绿植，再回到客厅沙发旁'
```
...
此外，`APPROACH_AND_ALIGN` 在几何停靠成功后会再采集 5 个近角度实时 RGB 视图，
运行不带类别清单的 Florence 检测，再用目标框的画面边距和尺度做 VLA handoff 门控。
目标贴边、被截断、过小或过大都会返回 `BAD_VIEWPOINT`，不会把旧的搜索可见性直接沿用
到机械臂执行时刻。
