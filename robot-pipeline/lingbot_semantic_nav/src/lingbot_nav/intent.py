...
class _JsonHttpClient:
    def __init__(self, timeout_seconds: float = 20.0, retries: int = 2) -> None:
        ...
        # Some Isaac/Conda launches do not ship a usable OpenSSL default CA
        # path even though the host trust store is present.  Prefer an
        # explicit operator override, otherwise use the standard Debian CA
        # bundle so DeepSeek does not silently fall back to keyword rules.
        ca_file = os.getenv("SSL_CERT_FILE", "").strip()
        if not ca_file and os.path.isfile("/etc/ssl/certs/ca-certificates.crt"):
