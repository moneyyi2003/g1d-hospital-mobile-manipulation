...
def _florence_backend(model_path: Path):
    ...
    def infer(image_path: Path, task: str) -> list[dict[str, Any]]:
        ...
        if task.startswith("<") and ">" in task:
            task_token = task[: task.index(">") + 1]
        image = Image.open(image_path).convert("RGB")
        views = [("full", image, 0, 0)]
        if task_token == "<DENSE_REGION_CAPTION>":
...
def create_florence_backend(model_path: Path):
    """Create one reusable Florence backend for discovery and grounding."""

    return _florence_backend(model_path)
