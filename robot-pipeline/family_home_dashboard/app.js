...
function render() {
  ...
  $("vlaPolicy").textContent = app.config?.manipulation?.policy || "—";
  $("executionBackend").textContent = app.config?.manipulation?.direct_vla_joint_control
    ? "VLA action 直控"
    : "OpenVLA 推理 → Expert DLS-IK 执行";
  $("hudAction").textContent = (state.action || "standby").toUpperCase();
  $("hudTiming").textContent = `FRAME ${String(state.frame || 0).padStart(4, "0")}`;
  $("robotHudTiming").textContent = `FRAME ${String(state.frame || 0).padStart(4, "0")}`;
  ...
  $("importMapButton").disabled = Boolean(state.process_running);
  ...
  if (cameraAvailable && !app.cameraStreamsStarted) {
    app.cameraStreamsStarted = true;
    $("cameraView").src = `${app.config.camera_stream}?session=${Date.now()}`;
    $("cameraView").style.display = "block";
    $("cameraEmpty").style.display = "none";
    $("robotCameraView").src = `${app.config.robot_camera_stream}?session=${Date.now()}`;
    $("robotCameraView").style.display = "block";
    $("robotCameraEmpty").style.display = "none";
  }
  drawMaps();
}
...
$("commandForm").addEventListener("submit", async (event) => {
  ...
  try {
    app.cameraStreamsStarted = false;
...
function renderLogs(payload) {
  ...
  const events = (payload.events || []).map((event) => {
    const details = Object.keys(event.details || {}).length
      ...
      : "";
    return `[${event.time}] ${String(event.kind).toUpperCase()} ${event.message}${details}`;
  });
  const processLines = payload.lines || [];
  const combined = [
    "=== PIPELINE EVENTS ===",
    ...events,
    "",
    "=== ISAAC / NAVIGATION / VLA OUTPUT ===",
    ...(processLines.length ? processLines : ["尚无任务进程日志。"]),
  ].join("\n");
  if (output.textContent !== combined) output.textContent = combined;
  if (nearBottom) output.scrollTop = output.scrollHeight;
  $("logState").textContent = `${payload.current?.state || "idle"} · ${payload.current?.action || "idle"}`.toUpperCase();
}

async function pollLogs() {
  try {
    app.logs = await request("/api/logs");
    renderLogs(app.logs);
  } catch (error) {
    $("logState").textContent = `LOG ERROR · ${error.message}`;
  }
  window.setTimeout(pollLogs, 1000);
}

$("importMapButton").addEventListener("click", () => $("mapBundleInput").click());

$("mapBundleInput").addEventListener("change", async (event) => {
  const file = event.target.files?.[0];
  if (!file) return;
  const status = $("importStatus");
  status.className = "";
  status.textContent = `正在上传并校验：${file.name}（${(file.size / 1024 / 1024).toFixed(1)} MiB）`;
  $("importMapButton").disabled = true;
  try {
    const payload = await request("/api/map-bundle", {
      method: "POST",
      headers: {
    ...
    $("robotCameraView").removeAttribute("src");
    $("cameraView").style.display = "none";
    $("robotCameraView").style.display = "none";
    $("cameraEmpty").style.display = "grid";
    $("robotCameraEmpty").style.display = "grid";
    await loadBundleData();
    status.className = "success";
    status.textContent = `导入完成：${payload.bundle_root}`;
  } catch (error) {
    status.className = "error";
    status.textContent = `导入失败：${error.message}`;
  } finally {
    $("importMapButton").disabled = Boolean(app.state?.process_running);
    event.target.value = "";
  }
});
...
async function initialize() {
  await loadBundleData();
  window.setTimeout(poll, 500);
  window.setTimeout(pollLogs, 250);
}
