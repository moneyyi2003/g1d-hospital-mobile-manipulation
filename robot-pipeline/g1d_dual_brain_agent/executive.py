...
class DualBrainExecutive:
    ...
    def _handle_failure(
        ...
        # A physical manipulation adapter may have moved the arm, fingers and
        # object before reporting failure.  Blindly replaying MANIPULATE makes
        # its cleanup/reset look like an unexplained teleport in the GUI and
        # can be unsafe on hardware.  Stop after that one physical attempt;
        # search and navigation failures retain their normal recovery policy.
        manipulation_details = result.details.get("manipulation", {})
        physical_execution = bool(
            result.details.get("physical_execution", False)
            or (
                isinstance(manipulation_details, dict)
                and manipulation_details.get("physical_execution", False)
            )
        )
        if kind is SkillKind.MANIPULATE and physical_execution:
            return (
                MissionStatus.FAILED,
                result.message,
                failure,
