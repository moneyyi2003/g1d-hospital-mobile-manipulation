...
def resolve_place(
    command: str, places: Sequence[Place], *, reference: Pose2D | None = None
) -> Place:
    """Resolve only cataloged places; coordinates never come from language.

    When several approved places tie on alias score (e.g. many office chairs),
    a `reference` pose picks the nearest one instead of raising.
    """
    ...
    if len(matches) > 1 and matches[0][0] == matches[1][0]:
        ...
        tied = [place for score, place in matches if score == best_score]
        return min(
            tied,
            key=lambda place: (place.pose.x - reference.x) ** 2
            + (place.pose.y - reference.y) ** 2,
        )
    return matches[0][1]
