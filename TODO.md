...
2026-08-14：OpenVLA-OFT 微调的软件与数据合同准备完成，最新 Family Home 第一视角
Expert 轨迹可被上游 OFT loader 读取；追加 5 次尝试后当前有 4 条成功轨迹、181 帧、
153 个 8-step 样本，
尚未达到项目正式训练门（100 条成功轨迹、4000 个 action-chunk），因此微调保持未启动。
下一步只批量采集并审计同一头部相机/动作合同下的成功轨迹，达到门槛后再启动训练。
新一轮已把近截断距离固定为 `0.1 m` 并单独采集到 2 条合格轨迹（97 帧、83 个
action-chunk）；后续正式批量数据应继续写入该 `clip01` 相机域，旧 `1.0 m` 数据不默认
混用。
...
2026-08-03 网页端已实跑完成导航、RGB 找杯、两级对齐、OpenVLA 推理、连续右手拿取
和携杯返回。G1-D/ROS 导航正向经 URDF `+X`、右臂工作空间和 4 段实际位移联合复核；
双手在行走时位于身体侧后方是收臂姿态，不代表底盘倒行。操作切换现先平滑进入前向弯肘
预抓取 seed，再执行有界位置 IK。近期仿真下一步仍是场景/自碰撞 IK 与抓取鲁棒性。
...
- [ ] **P0：OpenVLA-OFT 正式训练等待合格 Family Home Expert 数据。** OFT 环境、
  8-step/7D 数据适配、LoRA 连续 action head 和单卡 1-step 反向传播已在 2026-08-11
  通过；现有 8 条历史轨迹重新审核后仅 1 条合格（61 帧/54 samples），其余因大黑框或
  单步平移跳变被拒绝。近期唯一训练前置是用更新后的采集门重采至少 30 条物理成功、
  目标可见且无动作跳变的家庭场景轨迹，再运行正式训练；不得用重复单条轨迹宣称完成
  微调。
