...
if [ "${LIVING_ROOM_MODE:-survey}" = "viewer" ]; then
  exec /isaac-sim/python.sh /workspace/scripts/view_living_room.py
fi
exec /isaac-sim/python.sh /workspace/run_g1d_simple_room_vln.py \
  --scene-profile living-room \
  --survey \
  --output-dir /workspace/outputs/living_room_vln \
